...
- `quality-learning-adapter` - load when a ship or scout task worktree contains both `docs/workflows/quality-learning-harness.md` and `scripts/ci/check-quality-learning.py`, or when PR readiness or merge touches a task whose meta records `quality_learning=active`.
